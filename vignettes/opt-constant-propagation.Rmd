...
If it is an assignment of a constant value: then it returns the same expression, and saves `VAR = CONST` in `values`.

This criteria point also includes, multi-assign ( `VAR <- VAR <- CONST` ).

### `VAR * 3 + VAR`

If it is only operators, variables and constants: then it will check if any of the variables is stored in `values` vector, and would replace it in the expression. It will return the modified expression.

### `FUN({FUN_PARAMS})`

If it is a function call: then it returns the same expression, but trying to constant propagate in `{FUN_PARAMS}`, consider the case that the function call is `seq_len(x+30)`, it could be replaced by `seq_len(-3+30)`.
...
If it is an `if`: it will get which variables are being assigned in the if/else `BODY` and remove them from `values`, and then propagate in `COND` and `BODY`.

In-loop assigned variables are removed from `values`, as it is possible that the `BODY` is never executed (FALSE cond).
...
If it is a function definition: it will propagate on the `BODY` but with an empty `values` vector (the global `values` vector would not be changed). Note that in R, a function definition will have another environment, and thus it should not use the current `values`.
