...
Constant folding gets rid of unneeded calculations later on in the program. These calculations only use inputs that don't change, meaning their result will always be the same. To reduce the time needed to run these expressions later on, phrases like `320 * 200` are converted to `64000`; this means that the value doesn't have to be computed.
