...
Consider the same example to be automatically optimized.
...
* Finds all loops in the code, i.e., `r rco:::loops`.

* Removes loops that have function calls inside, as function calls can edit the environment.

* Gets which variables are variant inside the loop.

* Detects which expressions do not use any variant variable, and moves them outside the loop.
...
* Check that assigned vars that are moved, were present in parent environment, if not, add an `if`.
  
  Invariant expressions are moved outside of the loop independently if the loop conditions was `TRUE` or `FALSE`, this could lead to not equivalent codes, as for example in:
  ...
  The optimizer should check if `x` was previously assigned in code, and if not, optimize to:
