...
Dead code are pieces of code that do not affect the output of a program. Dead Code Elimination is an optimization technique that removes dead code, of which common examples include code that will never be executed known as unreachable code, or code that affects dead variables. Dead code elimination shrinks the program size, making the program run significantly faster. 
